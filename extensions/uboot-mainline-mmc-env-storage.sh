# @description Gives SPI-less boards a persistent mainline U-Boot environment, stored raw on the eMMC/SD card in the reserved gap that already exists ahead of the first partition. Sets `CONFIG_ENV_IS_IN_MMC` with a redundant copy, ships `libubootenv-tool`, and installs a boot-time generator for `/etc/fw_env.config` so `fw_printenv`/`fw_setenv` work from Linux. This is the eMMC counterpart of what boards like `nanopct6` and `rock-5b` already do in SPI NOR.
#
# Env location is chosen so that *no* `write_uboot_platform()` variant ever overwrites it:
#   rockchip64/rk35xx/rockchip-rk3588 (OFFSET=16 MiB): 0xE00000 (14 MiB) + redundant at 0xE20000, 0x20000 each.
#     idbloader lands at 32 KiB, u-boot.itb/uboot.img at 8 MiB, trust.bin at 12 MiB. On binman boards the single
#     u-boot-rockchip.bin is written at 32 KiB and is 0xff-padded up to CONFIG_SPL_PAD_TO (0x7f8000 on rk3588),
#     so it physically spans 32 KiB -> ~9.5 MiB. That is why mainline's own ENV_OFFSET default for
#     ARCH_ROCKCHIP && ENV_IS_IN_MMC (0x3f8000) is NOT used here: it would be erased on every u-boot reflash.
...
function extension_prepare_config__uboot_mainline_mmc_env_storage() {
	...
	# Known-good locations per family. Anything else must opt in explicitly by setting UBOOT_MMC_ENV_OFFSET.
	declare table_offset="" table_redund="" table_mode=""
	case "${BOARDFAMILY}" in
		rockchip64 | rk35xx | rockchip-rk3588)
			# 16 MiB gap; everything u-boot writes ends below ~13 MiB. rockchip's mmc_get_env_dev()
			# (arch/arm/mach-rockchip/board.c) makes u-boot follow the device SPL booted from, so userspace must too.
		...
		meson-gxbb | meson-gxl | meson-g12a | meson-g12b | meson-sm1 | meson-axg | meson-s4t7 | jethub)
			# 4 MiB gap by default; u-boot.bin from LBA 1. meson has no mmc_get_env_dev() override, so u-boot
			# always uses CONFIG_ENV_MMC_DEVICE_INDEX and userspace must point at that same fixed device.
	...
	declare -g UBOOT_MMC_ENV_FWENV_MODE="${UBOOT_MMC_ENV_FWENV_MODE:-"${table_mode}"}"
...
function post_config_uboot_target__900_uboot_mainline_mmc_env_storage() {
	...
	if [[ ! -f scripts/config ]]; then
		exit_with_error "${EXTENSION} requires u-boot with scripts/config (2022.10 or newer)" \
	...
	display_alert "u-boot for ${BOARD}/${BRANCH}" "${EXTENSION}: env in MMC at ${UBOOT_MMC_ENV_OFFSET}" "info"
	...
	run_host_command_logged scripts/config --set-val CONFIG_ENV_MMC_EMMC_HW_PARTITION "0" # user area, not boot0/boot1
...
# Called once per produced u-boot binary. Verify the binary we're about to ship does not reach into the env window.
...
function post_family_tweaks__uboot_mainline_mmc_env_storage() {
	display_alert "Configuring fw_printenv and fw_setenv" "${EXTENSION}: env in MMC for ${BOARD}" "info"
	...
	cat <<- 'FWENV_GENERATOR_BODY' >> "${generator}"
		...
		TMP=$(mktemp /etc/fw_env.config.XXXXXX)
		{
		    ...
		    echo "# Raw u-boot environment (redundant, two copies) in the reserved area before partition 1."
		    echo "# Device	Device offset	Env. size"
		    printf '%s\t%s\t%s\n' "${DEV}" "${ENV_OFFSET}" "${ENV_SIZE}"
		    printf '%s\t%s\t%s\n' "${DEV}" "${ENV_OFFSET_REDUND}" "${ENV_SIZE}"
		} > "${TMP}"
		chmod 0644 "${TMP}"
	...
	# Build-time copy, so fw_printenv works on the very first boot before the unit has run.
	{
		...
		printf '%s\t%s\t%s\n' "/dev/mmcblk${UBOOT_MMC_ENV_DEVICE_INDEX}" "${UBOOT_MMC_ENV_OFFSET}" "${UBOOT_MMC_ENV_SIZE}"
		printf '%s\t%s\t%s\n' "/dev/mmcblk${UBOOT_MMC_ENV_DEVICE_INDEX}" "${UBOOT_MMC_ENV_OFFSET_REDUND}" "${UBOOT_MMC_ENV_SIZE}"
