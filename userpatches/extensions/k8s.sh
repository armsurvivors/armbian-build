...
function pre_customize_image__800_k8s_itself() {
	...
	display_alert "Holding k8s packages to prevent accidental upgrades" "${EXTENSION} ${K8S_MAJOR_MINOR}" "info"
	chroot_sdcard_custom_with_apt_logic apt-mark hold kubeadm kubelet kubectl
...
function pre_customize_image__400_k8s_debfoster() {
	...
	# Obtain a list of packages debfoster would remove, but do not actually remove them yet. This is just for logging purposes.
	display_alert "Debfoster: list of packages that would be removed" "before debfoster" "info"
	chroot_sdcard_custom_with_apt_logic debfoster --force --option "'RemoveCmd=echo DEBFOSTER REMOVING '" -o "UseRecommends=no"

	# don't really do anything if K8S_DEBFOSTER==skip.
	if [[ "${K8S_DEBFOSTER}" == "skip" ]]; then
		display_alert "Debfoster: skipping actual removal of packages" "K8S_DEBFOSTER=skip" "info"
		return 0
	fi
